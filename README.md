...
```
🔍 Detect Signals → 📊 Calculate Risk → ⚠️ Identify Risk Level → 💡 Recommend Actions → 💬 Send Messages
```
...
```bash
git clone https://github.com/roge29/employee-burnout-agent.git
cd employee-burnout-agent
```
...
### Generate Messages with LLM (Not Included in my project but could be integrated)
